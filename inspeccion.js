...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function actualizarDetalleDesdeInput() {
        ...
        async function abrirCamaraParaIndice(idx, onCapture) {
            ...
            function nombreArchivoAutoSave() {
                try {
                    const equipo = (() => {
                        try {
                    ...
                    const mi = String(now.getMinutes()).padStart(2, '0');
                    const ss = String(now.getSeconds()).padStart(2, '0');
                    const idxPart = (idx === -1) ? 'OBS' : ((idx === -2) ? 'OBS2' : `P${String(idx)}`);
                    return `INSP-${eqSafe}-${y}${mo}${d}-${hh}${mi}${ss}-${idxPart}.jpg`;
                } catch {
                    return `INSP-${Date.now()}.jpg`;
                }
            }

            function intentarGuardarEnDispositivo(blob, filename) {
                try {
                    if (!blob) return;
                    const name = String(filename || '').trim() || `INSP-${Date.now()}.jpg`;
                    const url = URL.createObjectURL(blob);
                ...
                Promise.resolve(normalizarYComprimirImagen(fileOrBlob))
                    .then((b) => {
                        try {
                            ...
                            try { onCapture(fileOrBlob); } catch {}
                        }
                    })

...
function usage() {
  ...
  console.log('Roles: admin | director | supervisor | inspector | capturista | visor | none');
}
...
async function main() {
  ...
  const allowed = new Set(['admin', 'director', 'supervisor', 'inspector', 'capturista', 'visor', 'none']);
  if (!allowed.has(role)) {
